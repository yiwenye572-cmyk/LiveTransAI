...
function updateScenarioFieldsUi(scenarioDef) {
  ...
  setupGeneralFields.hidden = false;
  const isGeneral = scenarioDef.id === "general";
  if (scenarioInput) {
    scenarioInput.readOnly = !isGeneral;
  }
  if (instructionInput) {
    instructionInput.readOnly = !isGeneral;
  }
}
...
function expandScenario(scenarioDef, { apply = false } = {}) {
  ...
  if (scenarioDef) {
    updateScenarioFieldsUi(scenarioDef);
...
async function applyDemoScenario(scenarioDef) {
  ...
  const prior = loadSessionConfig();
  const scenarioChanged = prior?.scenario !== scenarioDef.scenario;
  ...
  if (!loadPregen) {
    if (scenarioChanged) {
      ...
      saveSessionConfig({ ...cfg, term_map: {} });
      renderPreview({ term_map: {} });
    }
    if (scenarioDef.id === "general") {
      setStatus(`已选择「${scenarioDef.label}」，请填写场景说明或生成术语表。`);
    } else if (scenarioChanged) {
      setStatus(
        `已选择「${scenarioDef.label}」场景描述；未加载预置术语，已清除上一场术语表。`
      );
    } else {
